...
POE1 / POE2 价格补丁工具是一个用于 Path of Exile 1 和 Path of Exile 2 的物品价格显示补丁生成和管理系统。该工具的主要功能是：

1. **自动获取物品价格**：POE1 国际服使用 poe.ninja、国服使用 poecurrency.top v1；POE2 保持 poe2scout / poecurrency.top v2，并按数据健康度使用兼容缓存和兜底源
2. **生成补丁文件**：将价格信息嵌入游戏数据文件中，使物品名称后显示价格（如 `Exalted Orb = 150D`）
3. **支持三类客户端**：POE1/POE2 均兼容国服 WeGame、国际服官方版（GGPK 格式）和国际服 Steam/Epic（Bundles2 格式）
4. **统一入口**：运行 `物价补丁.exe` 后在 GUI 内选择更新或还原；两种操作共用游戏目录校验，可自动识别、复用最近目录或由用户手动选择
