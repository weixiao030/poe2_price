...
`v0.4.9.6` 补齐更新与还原的游戏目录选择：补丁文件夹可以放在任意位置，两个入口都会自动识别或手动选择游戏目录，并保存最近一次有效选择。自动识别新增 WeGame 游戏库和国服正式名称“流放之路：降临”；发现多个客户端时不再静默选择，必须由用户确认。更新与还原还会按真实游戏目录加互斥锁，防止两个补丁副本同时写入同一客户端。

本版本同时修复 [Issue #23](https://github.com/weixiao030/poe2_price/issues/23)：国际服 `BaseItemTypes` 中多个元数据路径共用同一个显示名时，现在会把价格写入全部对应别名；国服遇到翻译重名时会使用英文别名消歧，避免价格串到其他物品。该修复已用最新版官服 `Content.ggpk` 验证。
...
解压 `物价补丁` 文件夹即可使用。放到 POE2 **游戏根目录**仍是最简单的方式，但现在也可以放在其它目录，并在更新窗口里自动识别或手动选择游戏文件夹：
...
1. **关闭游戏**
2. 双击 `一键更新物价补丁.exe`
3. 在弹窗中确认“自动识别游戏文件夹”，或切换到“手动选择游戏文件夹”；确认窗口显示的客户端类型和路径正确后，再选择本次要写入的补丁范围。“岛屿传言提示”默认勾选，如本次不需要可取消
4. 等待程序自动抓取价格并写入游戏；窗口中的 `[进度]` 会显示当前数据源、分类分页、重试和生成补丁包状态
5. *(可选)* 需要恢复原版时，双击 `一键还原物价补丁.exe`，在还原窗口中再次确认自动识别结果或手动选择要还原的客户端
