<p align="center">
  <h1 align="center">⚗️ POE2 物价补丁 v0.4.9.1</h1>
...
`v0.4.9.1` 重点修复了价格接口和 Bundles2 写入阶段“长时间卡住”的问题：网络请求超过硬截止时间后会自动切换备用源，已有增量补丁改为一次加载索引批量保留；写入前会检查游戏占用，写入后会读回文件校验 SHA256。同时加强了还原包版本兼容和失败回滚，避免把旧游戏索引还原到新版本客户端。
