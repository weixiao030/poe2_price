<p align="center">
  <h1 align="center">⚗️ POE1/2 物价补丁 v1.0.6</h1>
...
碑牌词缀分别使用魔法、稀有样本中位价：接近时合并，其余按从低到高显示，E/C/D 按同服汇率逐级切换，区间按低端选币种，小数截断不进位；合并标准随当前市场动态变化。提供“检查更新”、当前版本说明、赞赏码及聊天/bug反馈群:168887742；支持签名验证、差分或完整安装包下载、自动重启安装和下载失败重试。客户端通过公开 HTTPS 地址检查签名更新；上传配置和密钥不随软件分发。保留通货、传奇、岛屿传言、每小时物价更新及还原功能。
...
- POE2 国服：整件与暗金碑牌使用[国服整件碑牌 API](http://125.122.32.215:2083/api-docs#whole-tablets)，底材沿用国际服单价格式，优先普通报价，暗金按独立名称标价；原始币种样本不混算。碑牌词缀使用碑牌报价 API 的独立国服市场（`server=cn`），核对国服赛季、汇率、交易条件和实时分页修订；其余价格当前赛季主源 `poecurrency.top/api/summary?version=2`，历史赛季追加 `&season=<所选赛季>`，没有国服价的条目再用可用的国际参考源补。
...
- POE2 国服与国际服的通货、传奇、碑牌底材和词缀统一使用 E/C/D 阶梯显示：达到 1C 显示 C，达到 1D 显示 D，切换点随本服汇率变化。
- 价格最多显示两位小数，直接截断不四舍五入，去掉末尾零；极小有效价格保留到首个非零位，区间按低价端选择统一币种。
- 保留神圣石、崇高石及原有低价条目的不显示规则；价格来源与词缀合并规则保持原有行为。
- 国服碑牌名称改为单个底材参考价，优先普通报价，清理旧版普/魔/稀并列标记；保留暗金碑牌数据源及字体补丁译文。
- 优先最新行情，联网不可用时仍允许同赛季旧数据；升级后关闭游戏并更新一次物价。
...
- 保留原有颜色、双切译文、词缀数值档位和负数变换，兼容重复更新与还原。
- 当前资源不含本工具标记时刷新还原底板，避免旧备份覆盖新安装的汉化名称。
- 升级后先安装所需汉化或字体补丁，再关闭游戏更新一次物价补丁。

### 26/9/24 更新（v1.0.2）

- POE2 岛屿传言在地图名前追加 S/A/B/C 评级：星辰 S 级、四张 A 级、六张 B 级，其余首领图 C 级；颠沛领域、纯净乐园、千裂泽、顿悟时刻不评级。
- 修复国服“坠落的星辰”缺少八孔遗物提示，以及繁体首领名称；兼容旧提示升级、重复更新和清理还原。
- 升级后关闭游戏并更新一次物价补丁，也可选择“仅岛屿传言提示”应用修改。
