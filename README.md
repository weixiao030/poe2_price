...
选择国际服目录并手动开启后，在游戏前台世界地图上显示透明覆盖层，默认显示节点编号、地图名称和连接线。搜索结果显示距离及相对坐标、按距离排序，超过一页可以翻页浏览全部匹配项。关键词、手动起点、目标与路线模式自动保存；切换页面或重启后恢复，当前位置、节点状态或连接变化时自动重算路线。

点击“游戏内选起点”后切回游戏，可直接点击覆盖层节点；选完、右键取消或 20 秒未选择均恢复鼠标穿透。可调整编号、名称、隐藏节点、连线、透明度（10%–100%）、路线线宽与颜色，并选择允许后台显示（默认关闭）。普通路线支持人物到首节点的引导线，最优可开启节点路线不附加人物引导。重新识别布局会隐藏旧图，等连续有效帧后恢复；异常长连线会被过滤。关闭地图时隐藏覆盖层；切换应用页面、关闭到托盘不终止读取。
