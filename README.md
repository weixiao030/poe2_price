<p align="center">
  <h1 align="center">⚗️ POE1/2 物价补丁 v0.6.5</h1>
...
`v0.6.5` 新增 Windows 托盘常驻、开机自动启动和每小时自动更新物价；默认关闭，自动更新沿用最近一次成功手动补丁配置，并在游戏运行时自动跳过。此前版本的价格源、赛季刷新和启动反馈修复继续保留。
...
- 同步更新 PowerShell Worker、payload、启动器元数据、使用文档和完整更新日志，版本号统一保持 `0.6.5`。
