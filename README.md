<p align="center">
  <h1 align="center">⚗️ POE2 物价补丁 v0.4.9.2</h1>
...
`v0.4.9.2` 专注价格数据源的长期兼容性：自动识别当前软核赛季，单个可选分类异常时保留其他健康价格，并为四个数据源增加结构、分类、时效和参考通货健康校验。Poe2DB 间歇出现的单链接价格格、poecurrency 英文名与质量字段也已兼容，日常实时审计会直接请求数据源并报告结构漂移。
