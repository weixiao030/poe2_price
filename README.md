<p align="center">
  <h1 align="center">⚗️ POE1/2 物价补丁 v0.6.6</h1>
...
`v0.6.6` 新增 Windows 托盘常驻、开机自动启动和每小时自动更新物价；默认关闭，自动更新沿用最近一次成功手动补丁配置，并在游戏运行时自动跳过。此前版本的价格源、赛季刷新和启动反馈修复继续保留。
...
5. 在“后台运行设置”中点击“开机自启”和“每小时更新”按钮切换设置；绿色按钮表示“已启用”。开机自启后程序默认只驻留系统托盘，关闭主窗口不会结束后台任务。

发布包中的 `tools` 目录为明文脚本，启动器会直接从该目录加载，不再创建黑色控制台窗口、加密 payload 或临时解密目录。
