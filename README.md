<p align="center">
  <h1 align="center">⚗️ POE1/2 物价补丁 v0.6.2</h1>
...
`v0.6.2` 修复切换历史赛季时保留当前赛季旧价格标记的问题：历史赛季会先清理旧标记，再只写入该赛季实际获取并匹配的价格，避免 POE1/POE2 的不同赛季数据混用。`v0.6.1` 的 GUI 赛季控件位置修复，以及 `v0.6.0` 的运行时赛季发现、赛季标识配对和缓存隔离继续保留。
