...
`v0.6.3` 修复 poe.ninja 被 Cloudflare 间歇性返回 403 时直接判定数据源失败的问题：请求会按站点规则限并发，并尊重 `Retry-After` 做退避重试。`v0.6.2` 的历史赛季旧标记清理、`v0.6.1` 的 GUI 赛季控件位置修复，以及 `v0.6.0` 的运行时赛季发现、赛季标识配对和缓存隔离继续保留。
