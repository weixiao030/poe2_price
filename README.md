<p align="center">
  <h1 align="center">⚗️ POE2 物价补丁 v0.4.9.6</h1>
...
`v0.4.9.6` 修复 [Issue #23](https://github.com/weixiao030/poe2_price/issues/23)：国际服 `BaseItemTypes` 中多个元数据路径共用同一个显示名时，现在会把价格写入全部对应别名；国服遇到翻译重名时会使用英文别名消歧，避免价格串到其他物品。该修复已用最新版官服 `Content.ggpk` 验证。
...
- Bundles2 与 GGPK 写入增加写前并发指纹、写后读回校验和自动回滚；一键还原优先使用无需运行时的物理备份，逻辑还原失败会自动重试并复核。
...
- 修复 [Issue #19](https://github.com/weixiao030/poe2_price/issues/19)：`poe2scout` 大响应采用分块读取和硬墙钟截止时间，超时后不再无限等待，自动进入 `poe.ninja` / Poe2DB Economy 备用链路。

- 修复停在“使用 PatchBundle3 写入补丁到 Bundles2”的问题：已有 `LibGGPK3` 增量文件改为单次加载索引、批量提取，并显示扫描和合并进度。

- 写入 Bundles2 前会阻止游戏运行/索引占用；写入后会读回本次目标文件并逐项校验长度和 SHA256，避免工具退出成功但补丁实际未生效。

- 真实还原包绑定创建时的官方游戏底板；游戏更新后的旧备份会被拒绝。物理还原改为先完整校验、保留回滚副本再替换，失败时自动恢复。

- 修复跨运行复用旧 `物价补丁.zip` 导致未选中的通货或传奇补丁被重新安装；加强 BaseItemTypes 结构兼容签名，并防止同一游戏目录同时运行两个更新/还原实例。

- GUI 和可执行文件版本更新为 `v0.4.9.1`，发布构建补齐 BundleExtractor VC++ 运行库并加入自动测试。
