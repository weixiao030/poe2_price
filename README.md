<p align="center">
  <h1 align="center">⚗️ POE1/2 物价补丁 v0.6.3</h1>
...
`v0.6.3` 修复 poe.ninja 被 Cloudflare 间歇性返回 403 时直接判定数据源失败的问题：请求会按站点规则限并发，并尊重 `Retry-After` 做退避重试。2026/9/5 主分支又修复了赛季刷新默认值：每次打开工具、切换 POE1/POE2 客户端或点击刷新都会重新读取赛季目录，默认选中最新软核赛季；新旧赛季交接时也只显示一个“（最新）”标记。`v0.6.2` 的历史赛季旧标记清理、`v0.6.1` 的 GUI 赛季控件位置修复，以及 `v0.6.0` 的运行时赛季发现、赛季标识配对和缓存隔离继续保留。
