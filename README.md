<p align="center">
  <h1 align="center">⚗️ POE1/2 物价补丁 v1.0.1</h1>
...
每次启动会在后台检查软件版本。发现新版时，右上角显示约 3 秒的提示；悬停或键盘聚焦时保留提示。可点击“查看更新”手动操作，或选择“本次更新不再提醒”。忽略设置只针对该版本，之后的新版本仍会提示；不会自动下载或安装。
...
- 接入 POE2 国服碑牌词缀行情，魔法与稀有报价按国服汇率显示，服务器、赛季、稀有度缓存独立。
- 支持国服 API 已校验的实时行情；游戏表和交易条件共同绑定词缀，缺价保留原文。
- 每次启动后台检测软件更新，右上角约 3 秒非阻塞提示，可按版本选择不再提醒，不自动下载或安装。
...
- 修复碑牌价格导致过滤器报错：英文底材保留原名，恢复 `BaseType == "Breach Tablet"` 等精确匹配。
- 自动清除旧版英文碑牌价签；保留中文名称价格和词缀价格，安装前检查所有英文底材价格标记。
- 提供完整安装版、免安装 ZIP，以及 0.9.2 增量包和兼容两种 0.9.3 的完整更新包。升级后关闭游戏并更新一次物价补丁。
...
- 新增在线更新功能。
- 新增碑牌价格和碑牌词缀价格，价格为魔法和稀有的单词条最低价
- 优化碑牌词缀价格为稀有~魔法价格，使用动态算法计算价格。
- 魔法、稀有分别取在售样本中位价，价格接近就合并，差距较大就显示从低到高的区间。
- 当前只有POE2有碑牌价格，国服后续更新。
