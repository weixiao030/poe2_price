...
const provenance = JSON.parse(
  await fs.readFile(path.join(root, 'docs/core-provenance.json'), 'utf8')
)
for (const record of provenance.files) {
  ...
  const bundled = await fs.readFile(
    path.join(desktop, '.runtime', record.path.replace(/^物价补丁\//, ''))
  )
  assert.deepEqual(bundled, source, `Packaged core differs: ${record.path}`)
}
for (const seed of ['国服还原包.zip', '国际服还原补丁.zip'])
  assert.deepEqual(
    await fs.readFile(path.join(root, 'restore-seeds', seed)),
    await fs.readFile(path.join(desktop, '.runtime', seed))
  )
for (const record of provenance.native_tools) {
  ...
  if (record.path.startsWith('物价补丁/'))
    assert.deepEqual(
      await fs.readFile(path.join(desktop, '.runtime', record.path.replace(/^物价补丁\//, ''))),
      source
    )
}
...
console.log(
  JSON.stringify({
    ...
    upstreamEquivalentCoreFiles: provenance.files.filter((record) => record.equivalent).length,
    locallyChangedCoreFiles: provenance.files.filter((record) => !record.equivalent).length,
    unchangedNativeToolsAndSeeds: provenance.native_tools.length,
    restoreSeeds: 2,
    obsoleteEntrypoints: 0
  })
)
