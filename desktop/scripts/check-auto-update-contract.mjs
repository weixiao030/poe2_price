...
function functions(file, names) {
  ...
  return source.statements
    .filter((n) => ts.isFunctionDeclaration(n) && (!names || names.includes(n.name?.text)))
    .map((n) => n.getText(source).replace(/^export /, ''))
    .join('\n')
}
let state = {
  settings: { autoUpdate: true, gameVersion: 'poe2', directories: { poe2: 'C:\\Fixture\\POE2' } },
  history: [],
  confirmed: null
}
...
const context = vm.createContext({
  ...
  setTimeout: (callback, delay) => ({ callback, delay, unref() {} }),
  clearTimeout() {},
  refresh() {},
  send() {},
  log: {
