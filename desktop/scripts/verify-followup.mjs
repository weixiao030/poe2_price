...
const output = path.resolve(
  root,
  `../verification/desktop-v${version}/${packaged ? 'packaged-ui' : 'ui'}`
)
...
const svg = Buffer.from(
  '<svg width="3200" height="1800" xmlns="http://www.w3.org/2000/svg"><rect width="3200" height="1800" fill="#164966"/><path d="M0 1700L900 360L2000 1500L2500 600L3200 1600V1800H0" fill="#a88259"/><circle cx="2500" cy="380" r="170" fill="#f4d194"/></svg>'
)
...
async function launch() {
  app = await electron.launch({
    ...(packaged
      ? { executablePath: path.join(root, 'dist/win-unpacked/物价补丁.exe'), args: [] }
      : { args: [root] }),
    env,
    timeout: 60000
  })
...
async function choose(file) {
  await app.evaluate(({ dialog }, file) => {
    dialog.showOpenDialog = async () => ({ canceled: !file, filePaths: file ? [file] : [] })
  }, file)
}
...
try {
  ...
  await page.getByRole('heading', { name: '物价补丁', exact: true }).waitFor()
  await page.evaluate(() =>
    window.desktop.saveSettings({ closeToTray: false, autoUpdate: false, theme: 'light' })
  )
  ...
  await waitFor(
    async () =>
      (await page
        .locator('.theme-root')
        .evaluate((el) => getComputedStyle(el).getPropertyValue('--background-opacity').trim())) ===
      '0.45'
  )
  ...
  for (const file of ['invalid.png', 'large.png', 'unsupported.webp']) {
    ...
    const error = await page.evaluate(async () => {
      try {
        await window.desktop.chooseBackground()
        return ''
      } catch (e) {
        return String(e)
      }
  ...
  assert.equal(
    (await page.evaluate(() => window.desktop.getSnapshot())).settings.backgroundOpacity,
    45
  )
  await page.locator('.custom-background').waitFor()
