...
test('hourly preference survives operations; only manual off stops future checks', () => {
  ...
  assert.deepEqual(observed.beforeFirstUpdate, { enabled: true, confirmed: false, scheduled: false })
  for (const phase of ['enabledAfterSuccess', 'afterRestore', 'afterFailedHour', 'afterSkippedHour'])
